...
const getAllLichChayService = async () => {
  try {
    const result = await LichChay.find().populate('MaTuyen').populate('MaPT');
...
const createLichChayService = async (
  parternId,
  MaPT,
  MaTuyen,
  ngayKhoiHanh,
...
const updateLichChayService = async (
  id,
  MaPT,
  MaTuyen,
  ngayKhoiHanh,
  ...
  try {
    const result = await LichChay.updateOne(
      { _id: id },
      {
        MaPT: MaPT,
        MaTuyen: MaTuyen,
