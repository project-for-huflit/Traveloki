...
import {deleteLichChay, fetchAllLichChay} from "../../services/api/LichChay/apiDanhSachLichChay.js";
    ...
    if (lichChayToDelete) {
      try {
        ...
        if (res && res.EC === 0) {
          ...
          setLichChay((prev) => prev.filter((lichChay) => lichChay._id !== lichChayToDelete._id)); // Cập nhật danh sách sau khi xóa
        } else {
          alert(res.EM);
        }
      } catch (error) {
    ...
    <div className="w-auto h-full bg-white p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/schedule/list/create">
          <Button variant="contained" color="primary">Thêm lịch chạy</Button>
        </Link>
      </div>
        ...
        <Table>
          <TableHead>
            <TableRow sx={{ backgroundColor: '#e3f2fd' }}>
              <TableCell sx={{ color: '#1a73e8', fontWeight: 'bold' }}>STT</TableCell>
          ...
          <TableBody>
            {lichChay.map((lichChay, index) => (
              <TableRow key={lichChay._id} sx={{ '&:hover': { backgroundColor: '#e3f2fd' } }}>
